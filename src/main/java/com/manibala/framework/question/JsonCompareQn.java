...
public class JsonCompareQn {
    public String getJsonStringFromResponse(Response response) {
        return response.getBody().asString();
    }

    private JSONObject getJsonObjectFromResponse(Response response) {
        return new JSONObject(response.getBody().asString());
    }

    private JSONObject getJsonObjectFromJsonStr(String jsonStr) {
        return new JSONObject(jsonStr);
    }

    private JSONObject maskFieldInArray(JSONObject jsonObject1, String fieldToIgnore) {
    ...
    private JSONObject maskNestedField(JSONObject jsonObject, String fieldToIgnore) {
        try {
            String[] fields = fieldToIgnore.split("\\.");
    ...
    private JSONObject maskDirectField(JSONObject jsonObject, String field) {
        try {
    ...
    private String ignoreFields(Response response, List<String> fields) {
        return ignoreFields(getJsonObjectFromResponse(response), fields);
    }

    private String ignoreFields(String jsonStr, List<String> fields) {
        return ignoreFields(getJsonObjectFromJsonStr(jsonStr), fields);
    }

    private String ignoreFields(JSONObject jsonObject1, List<String> fieldsToIgnore) {
        try {
    ...
    public boolean compareJsonWithMmatchExpected(Actor actor, String message, String jsonStr1, String jsonStr2, List<String> fieldsToIgnore) {
        return compareJson(actor, message, ignoreFields(jsonStr1, fieldsToIgnore), ignoreFields(jsonStr2, fieldsToIgnore), true, true);
    }
